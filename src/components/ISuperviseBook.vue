              ...
              <template v-if="record.contentSinglBook && record.contentSinglBook.length">
                <div class="pervise-single-wrap">
                  <taskInfo
                    ...
                    @handleFileOpen="handleFileOpen"></taskInfo>
                </div>
              </template>
                    ...
                    <div class="task-li-box">
                      ...
                      <div class="task-action">
                        <a-popover placement="bottomRight" v-if="item.buttonList && item.buttonList.length">
                          <template slot="content">
                            <p v-for="(subitem, index) in item.buttonList" :key="index" @click="handleOptions({item: subitem, fatherItem: item})">
...
export default {
  ...
  methods: {
    ...
    changeFileByConvert(sourcePk, convertId, urlParam) {
      window.IDM.http.post("ctrl/convert/moduleRuleConvert", {
          ...
          if ("success" == result.type) {
              ...
              var url = window.DSF.getURLRoot() + "ctrl/formControl/form?moduleId=" + data.moduleId + "&pk=" + data.pk + "&sourcePk=" + sourcePk + "&relationId=" + sourcePk ;
    ...
    handleOptions(obj) {
      if(this.propData.handleActionFunc && this.propData.handleActionFunc.length > 0) {
        ...
        switch(item.value) {
          ...
          case 'task_urge':{ // 任务催办
            let id = '';
            if (fatherItem.isSingleTask == 1) { // 单任务立项
