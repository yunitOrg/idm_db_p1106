<template>
  ...
  -->
    <div idm-ctrl="idm_module" :id="moduleObject.id" :idm-ctrl-id="moduleObject.id" v-if="authPower">
            ...
            <div class="task-title" v-if="propData.titleShow == 'show'">
                <div>
                    <template v-if="propData.titleIconShow == 'show'">
                        <svg v-if="propData.titleIcon && propData.titleIcon.length" class="task-title-icon" aria-hidden="true">
                ...
                <span class="task-right-icon">
                    ...
                    <template v-if="propData.titleRightModule == 'svgList'">
                        <svg v-for="(item, index) in propData.titleRightGroup" :style="setIconStyle('titleIcon', item)" :key="index" @click="handleSvgClick(item)">
                            <use :xlink:href="`#${item?.titleBtnIcon && item.titleBtnIcon[0]}`"></use>
                        </svg>
                    </template>
                    <a-switch v-if="propData.showHideCloumn == 'show'" default-checked @change="onSwitchChange" />
                </span>
            </div>
            <!--列表-->
            <div class="task-ul">
                                ...
                                <div class="task-li-title">
                                    <div class="task-li-titleTop" @click.stop="handleShowDetail(item)">
                                        <template v-if="judgeComState">
                                            ...
                                            <span class="task-li-tip" :class="item.dbStatus == '4' || item.dbStatus == '5' ? 'taskbtn2' : 'taskbtn1'" v-if="item.dbStatusText">{{
                                                item.dbStatusText
                                            }}</span>
                                        </template>
    ...
    methods: {
                            ...
                            window.IDM.http[this.propData.requestType || 'get'](
                                ...
                                },
                                {
                                    headers: this.propData.requestContentType
                                        ? {
                                              'Content-Type': this.propData.requestContentType || 'application/json;charset=UTF-8'
                                          }
                                        : {}
                                }
                            ).then((res) => {
        ...
        getChildrenContextToThis() {
            ...
            this.list.forEach((item) => {
                const allModulePackageId = this.moduleObject?.getModuleChildrenPackageId(this.moduleObject.packageid, item[this.indexDataFiled], false)
                //所有返回结果
                let moduleAllData = window.IDM.broadcast.getModuleContextValue(allModulePackageId, null)
        ...
        renderDragComponent() {
            this.list.forEach((item, index) => {
                this.moduleObject.dynamicRenderModuleGroupInitData &&
                    this.moduleObject.dynamicRenderModuleGroupInitData(this.moduleObject.packageid, item[this.indexDataFiled], item, false, this.propData.sendMessageKey)
            })
        },
        init() {
            this.handleStyle()
